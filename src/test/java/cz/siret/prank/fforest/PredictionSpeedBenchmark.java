...
public class PredictionSpeedBenchmark {
    ...
    static final boolean ENABLE_LEGACY_FLAT_OPT    = true;
    static final boolean ENABLE_SHORT_LEGACY       = true;
    static final boolean ENABLE_SUPER_SHORT        = true;
    ...
    static final int ITERS_PER_ROUND = 200;
    ...
    LegacyFlatBinaryForest flatForest;
    ...
    public void init() throws Exception {
        ...
        if (ENABLE_LEGACY_FLAT || ENABLE_LEGACY_FLAT_OPT || ENABLE_SHORT_LEGACY || ENABLE_SUPER_SHORT) {
            flatForest = ff.toFlatBinaryForest();
        }

        if (ENABLE_LEGACY_FLAT_OPT && flatForest != null) {
            OptimizingFlatBinaryForest optimizingForest = new OptimizingFlatBinaryForest(flatForest);
        ...
        if (ENABLE_SHORT_LEGACY && flatForest != null) {
            shortForest = ShortLegacyFlatBinaryForest.from(flatForest);
        }
        if (ENABLE_SUPER_SHORT && flatForest != null) {
            superShortForest = SuperShortLegacyFlatBinaryForest.from(flatForest);
        }
    ...
    private LinkedHashMap<String, BinaryForest> buildForestMap() {
        ...
        if (ENABLE_LEGACY_FLAT)    forests.put("LegacyFlat", flatForest);
